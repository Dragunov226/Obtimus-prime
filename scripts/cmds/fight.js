...
module.exports = {
  ...
  onStart: async function ({ event, message, api, usersData, args }) {
    ...
    if (ongoingFights.has(threadID)) {
      return message.send("âš”ï¸ A fight is already in progress in this group.");
    }

    const mention = Object.keys(event.mentions);

    if (mention.length !== 1) {
      return message.send("ðŸ¤” Please mention one person to start a fight with.");
    }
  ...
  onChat: async function ({ event, message }) {
    ...
    if (gameInstance.lastAttack !== null && !isCurrentPlayer) {
      message.reply(
        `ðŸ˜’ It's ${currentPlayer.name}'s turn currently. You can't attack until they make a move.`
      );
      return;
    }
    ...
    if (!isCurrentPlayer && gameInstance.lastPlayer.id === event.senderID) {
      message.send(
        `ðŸ‘Ž It's ${currentPlayer.name}'s turn currently. You can't attack until they make a move.`
      );
      return;
    }
    ...
    if (attack === "forfeit") {
      ...
      message.send(`ðŸƒ ${forfeiter} forfeits the match! ${opponent} wins!`);
      endFight(threadID);
      ...
      if (opponent.hp <= 0) {
        ...
        message.send(`â° Time's up! The game is over. ${winner} wins! ${loser} is defeated.`);
        endFight(threadID);
      } else {
        ...
        message.send(`ðŸ¥² It's ${newCurrentPlayer.name}'s turn currently.`);
      }
    } else {
      message.reply(
        "âŒ Invalid attack! Use 'kick', 'punch', 'slap', or 'forfeit'."
      );
    }
  }
