...
module.exports = {
  ...
  onStart: async function ({ message, args }) {
    ...
    if (!username) {
      return message.reply("âš ï¸ | Please enter a username!");
    } else if (!["std", "taiko", "catch", "mania"].includes(mode)) {
      return message.reply("âš ï¸ | Please enter a valid mode (std, taiko, catch, mania)!");
    } else if (!["en", "cn"].includes(language)) {
      return message.reply("âš ï¸ | Please enter a valid language (cn, en)!");
    } else {
      try {
